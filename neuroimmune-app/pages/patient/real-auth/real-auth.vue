	...
	<view class="container">
		...
		<view class="card" v-if="currentBinding">
			<view class="binding-header">
				...
				<text class="binding-status active">生效中</text>
			</view>
			<view class="binding-info">
				<text class="binding-name">{{ currentBinding.doctorName }}</text>
				<text class="binding-time">绑定时间：{{ formatDate(currentBinding.bindTime) }}</text>
			</view>
	...
	methods: {
		...
		async submit() {
			...
			try {
				...
				// 更新本地存储
				uni.setStorageSync('doctorBound', true);
				uni.setStorageSync('doctorName', this.selectedDoctor.name);
				uni.setStorageSync('doctorId', this.form.doctorId);

				const u = uni.getStorageSync('userInfo') || {};
				u.isRealAuth = true;
				u.realName = this.form.realName;
				u.idCard = this.form.idCard;
				u.doctorId = this.form.doctorId;
				u.doctorName = this.selectedDoctor.name;
				uni.setStorageSync('userInfo', u);

				uni.showToast({ title: '认证成功，已绑定医生', icon: 'success' });
